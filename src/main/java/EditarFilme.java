...
public class EditarFilme {
    ...
    private JLabel erroDuracaoLabel = new JLabel();
    ...
    //construtor
    public EditarFilme(AppWindow app, Filme filme) {
        this.app = app;
    ...
    private void configurarComponentes(Filme filme) {
        ...
        List<Filme> filmesExistentes = BaseDados.getInstance().getFilmes(); // lista de todos os filmes existes na BD e ficheiro binario

        List<Filme> listaFilmesAEditar = new ArrayList<>();  // lista de todos os filmes como mesmo nome de 'filme'
        ...
        // ---------- Mostrar imagem inicial se existir ----------
        URL resource = getClass().getResource("/imagens/cartazes/" + filme.getFoto());
        if (resource != null) {
            try {
                ...
                if (img != null) {
                    ...
                    imagemCarregada = img;
                    ficheiroImagemOriginal = new File(resource.toURI()); // só se precisares do File depois
                    nomeFicheiroImagem = filme.getFoto();
                }
            } catch (Exception e) {
                System.err.println("Erro ao carregar imagem inicial: " + e.getMessage());
            }
        } else {
            uploadButton.setText("Carregar Cartaz");
        }
                ...
                if (result == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        if (img != null) {
                            ...
                            imagemCarregada = img;
                            ficheiroImagemOriginal = selectedFile;
        ...
        //NAO PERMITE ESCREVER MAIS DO QUE 35 CARACTERES NO CAMPO
        ((AbstractDocument) nomeFilme.getDocument()).setDocumentFilter(new DocumentFilter() {
        ...
        nomeFilme.setForeground(corFontePreto); // texto
        ...
        duracaoFilme.setText("" + filme.getDuracao());
        duracaoFilme.setForeground(corFontePreto); // texto
        ...
        // Criar painel com os checkboxes
        ...
        for (JCheckBox cb : checkBoxes) {
            ...
            cb.setIconTextGap(30);
            cb.setBorder(BorderFactory.createEmptyBorder(10, 5, 0, 0));
            checkBoxIdioma.add(cb);
        }

        // Criar popup arredondado
        ...
        // Atualizar texto do botão com nomes legíveis (Português, Inglês)
        Runnable atualizarTextoBotaoIdioma = () -> {
            ...
            for (JCheckBox c : checkBoxes) {
                if (c.isSelected()) {
                    ...
                    if (parenIndex != -1) {
                        sb.append(text, 0, parenIndex).append(", ");
                    } else {
            ...
            if (sb.length() > 0) {
                ...
                comboButtonIdioma.setText(sb.toString().trim());
            } else {
        ...
        for (JCheckBox cb : checkBoxes) {
            cb.addItemListener(e -> atualizarTextoBotaoIdioma.run());
        }

        // Obter os idiomas associados a todos os filmes com o mesmo nome
        Set<Idioma> idiomasSelecionados = new HashSet<>();
        for (Filme f : listaFilmesAEditar) {
            idiomasSelecionados.add(f.getIdiomas());
        }

        // Selecionar os checkboxes correspondentes
        for (JCheckBox cb : checkBoxes) {
            if (cb.getText().contains("VP") && idiomasSelecionados.contains(Idioma.VP)) {
                cb.setSelected(true);
            } else if (cb.getText().contains("VO") && idiomasSelecionados.contains(Idioma.VO)) {
                cb.setSelected(true);
            }
        }

        // Inicializar o texto do botão de idioma com base nos selecionados
        atualizarTextoBotaoIdioma.run();
        ...
        // mostra oa idade selecionada
        comboBoxIdade.setSelectedItem(filme.getIdade());
        ...
        // Mapear nomes dos checkboxes para enums
        Map<String, Genero> generoMap = new HashMap<>();
        generoMap.put("Animação", Genero.ANIMACAO);
        generoMap.put("Ação", Genero.ACAO);
        generoMap.put("Comédia", Genero.COMEDIA);
        generoMap.put("Terror", Genero.TERROR);
        generoMap.put("Romance", Genero.ROMANCE);
        generoMap.put("Ficção Científica", Genero.FICCAO_CIENTIFICA);

        // Selecionar os checkboxes com base nos géneros encontrados
        for (JCheckBox cb : checkBoxesGenero) {
            Genero g = generoMap.get(cb.getText());
            if (g != null && filme.getGeneros().contains(g)) {
                cb.setSelected(true);
            }
        }
        ...
        // Selecionar os tipos dos filmes a editar
        Set<String> tiposSelecionados = new HashSet<>();
        for (Filme f : listaFilmesAEditar) {
            if (f.getTipos() != null) {
                tiposSelecionados.add(f.getTipos().trim());
            }
        }

        // Marcar checkboxes correspondentes
        for (JCheckBox cb : checkBoxesTipo) {
            if (tiposSelecionados.contains(cb.getText())) {
                cb.setSelected(true);
            }
        }
        ...
        if (filme.getEstado() != null) {
            String estadoFormatado = switch (filme.getEstado()) {
                case ATIVO -> "Ativo";
                case INATIVO -> "Inativo";
            };
            comboBoxEstado.setSelectedItem(estadoFormatado);
        }
        ...
        if (filme.getPrecoCompra() > 0) {
            String precoFormatado = String.format("%.2f €", filme.getPrecoCompra()).replace(".", ",");
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                for (JCheckBox checkBox : checkBoxes) {
                    if (checkBox.isSelected()) {
                        if (checkBox.getText().contains("VP")) idiomas.add(Idioma.VP);
                        else if (checkBox.getText().contains("VO")) idiomas.add(Idioma.VO);
                    }
                }
                ...
                for (JCheckBox checkBox : checkBoxesGenero) {
                    if (checkBox.isSelected()) {
                        String normalizado = NormalizarEnum.normalizarEnum(checkBox.getText());
                ...
                // Gerar novos filmes com todas as combinações
                List<Filme> novosFilmes = new ArrayList<>();
                for (Idioma idioma : idiomas) {
                    for (String tipo : tipos) {
                        novosFilmes.add(new Filme(
                                nome, duracao, foto, idioma, idade, generos, tipo, estado, precoCompra
                        ));
                    }
                }

                // Verificar se houve alterações em relação à lista de filmes a editar
                boolean houveAlteracao = false;

                if (listaFilmesAEditar.size() != novosFilmes.size()) {
                    houveAlteracao = true;
                } else {
                    for (int i = 0; i < listaFilmesAEditar.size(); i++) {
                        Filme antigo = listaFilmesAEditar.get(i);
                        Filme novo = novosFilmes.get(i);
                ...
                if (!houveAlteracao) {
                    JOptionPane.showMessageDialog(null, "Não foi detetada qualquer alteração.", "Aviso", JOptionPane.INFORMATION_MESSAGE);
                    return;
                }

                // Remover filmes antigos
                bd = BaseDados.getInstance();
                for (Filme antigo : listaFilmesAEditar) {
                    bd.removerFilme(antigo);
                }

                // Adicionar novos
                for (Filme novo : novosFilmes) {
                    bd.adicionarFilme(novo);
                }
                ...
                // Guardar imagem
                try {
                    File destino = new File("src/main/resources/imagens/cartazes/" + ficheiroImagemOriginal.getName());
                ...
                // Redirecionar
                app.mostrarConfirmacaoEdicaoFilmes();
